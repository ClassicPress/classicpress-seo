...
class Admin_Bar_Menu {
	...
	public function mark_page_as() {
		...
		if ( 'pillar_content' === $what ) {
			$current = $this->get_meta( $object_type, $object_id, 'cpseo_pillar_content' );
			$updated = 'on' === $current ? 'off' : 'on';
			$this->update_meta( $object_type, $object_id, 'cpseo_pillar_content', $updated );
			die( '1' );
		}
	...
	private function add_mark_page_menu() {
		...
		if ( is_singular( Helper::get_accessible_post_types() ) ) {
			if ( get_post_meta( get_the_ID(), 'cpseo_pillar_content', true ) === 'on' ) {
			...
			$this->add_sub_menu(
				'pillar-content',
				[
					'title' => $is_pillar_content . esc_html__( 'As Pillar Content', 'cpseo' ),
					'href'  => '#pillar_content',
	...
	private function add_seo_tools() {
		...
		$items = [
			'google-pagespeed'           => [
				...
				'meta'  => [ 'title' => esc_html__( 'Google PageSpeed Insights', 'cpseo' ) ],
			],
			...
			'google-mobilefriendly'      => [
				...
				'meta'  => [ 'title' => esc_html__( 'Google Mobile-Friendly Test', 'cpseo' ) ],
			],
			...
			'google-richresults-mobile'  => [
				...
				'meta'  => [ 'title' => esc_html__( 'Google Rich Results Test - Googlebot Smartphone', 'cpseo' ) ],
			],
	...
	public function add_sub_menu( $id, $args, $parent = '' ) {
		...
		$args['id']         = 'cpseo-' . $id;
		$args['parent']     = '' !== $parent ? 'cpseo-' . $parent : self::MENU_IDENTIFIER;
		$this->items[ $id ] = $args;
	}
