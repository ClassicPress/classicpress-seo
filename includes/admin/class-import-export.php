...
class Import_Export implements Runner {
	...
	public function hooks() {
		...
		$this->filter( 'cpseo/export/settings', 'export_other_panels', 10, 2 );
		$this->action( 'cpseo/import/settings/pre_import', 'run_backup', 10, 0 );
	...
	public function register_page() {
		new Page(
			'cpseo-import-export',
			esc_html__( 'Import &amp; Export', 'cpseo' ),
			[
				'position' => 99,
				'parent'   => 'cpseo',
				...
				'assets'   => [
					...
					'scripts' => [ 'cpseo-import-export' => cpseo()->plugin_url() . 'assets/admin/js/import-export.js' ],
				],
			]
		);
	...
	public function handler() {
		...
		if ( 'export-plz' === $object_id && check_admin_referer( 'cpseo-export-settings' ) ) {
			$this->export();
		}

		if ( isset( $_FILES['import-me'] ) && 'import-plz' === $object_id && check_admin_referer( 'cpseo-import-settings' ) ) {
			$this->import();
		}
	}
	...
	public function clean_plugin() {
		$this->verify_nonce( 'cpseo-ajax-nonce' );

		$result = Detector::run_by_slug( Param::post( 'pluginSlug' ), 'cleanup' );

		if ( $result['status'] ) {
			/* translators: Plugin name */
			$this->success( sprintf( esc_html__( 'Cleanup of %s data successfully done.', 'cpseo' ), $result['importer']->get_plugin_name() ) );
		}

		/* translators: Plugin name */
		$this->error( sprintf( esc_html__( 'Cleanup of %s data failed.', 'cpseo' ), $result['importer']->get_plugin_name() ) );
	}
	...
	public function import_plugin() {
		...
		if ( ! $this->is_action_allowed( $perform ) ) {
			$this->error( esc_html__( 'Action not allowed.', 'cpseo' ) );
		}
	...
	public function create_backup() {
		...
		$key = $this->run_backup();
		if ( is_null( $key ) ) {
			$this->error( esc_html__( 'Unable to create backup this time.', 'cpseo' ) );
		}

		$this->success([
			'key'     => $key,
			/* translators: Backup formatted date */
			'backup'  => sprintf( esc_html__( 'Backup: %s', 'cpseo' ), date_i18n( 'M jS Y, H:i a', $key ) ),
			'message' => esc_html__( 'Backup created successfully.', 'cpseo' ),
		]);
	}
	...
	public function delete_backup() {
		...
		$key = Param::post( 'key' );
		if ( ! $key ) {
			$this->error( esc_html__( 'No backup key found to delete.', 'cpseo' ) );
		}

		$this->run_backup( 'delete', $key );
		$this->success( esc_html__( 'Backup successfully deleted.', 'cpseo' ) );
	}
	...
	public function restore_backup() {
		...
		$key = Param::post( 'key' );
		if ( ! $key ) {
			$this->error( esc_html__( 'No backup key found to restore.', 'cpseo' ) );
		}

		if ( ! $this->run_backup( 'restore', $key ) ) {
			$this->error( esc_html__( 'Backup does not exist.', 'cpseo' ) );
		}

		$this->success( esc_html__( 'Backup restored successfully.', 'cpseo' ) );
	}
	...
	public function run_backup( $action = 'add', $key = null ) {
		...
		update_option( 'cpseo_backups', $backups );
	...
	private function export() {
		...
		$filename = 'cpseo-settings-' . date_i18n( 'Y-m-d-H-i-s' ) . '.txt';
	...
	private function import() {
		...
		if ( $this->do_import_data( $settings ) ) {
			Helper::add_notification( esc_html__( 'Settings successfully imported. Your old configuration has been saved as a backup.', 'cpseo' ), 'success' );
			return;
		}

		Helper::add_notification( esc_html__( 'No settings found to be imported.', 'cpseo' ), [ 'type' => 'info' ] );
	}
	...
	private function has_valid_file() {
		...
		if ( is_wp_error( $file ) ) {
			Helper::add_notification( esc_html__( 'Settings could not be imported:', 'cpseo' ) . ' ' . $file->get_error_message(), [ 'type' => 'error' ] );
			return false;
		}

		if ( isset( $file['error'] ) ) {
			Helper::add_notification( esc_html__( 'Settings could not be imported:', 'cpseo' ) . ' ' . $file['error'], [ 'type' => 'error' ] );
			return false;
		}

		if ( ! isset( $file['file'] ) ) {
			Helper::add_notification( esc_html__( 'Settings could not be imported: Upload failed.', 'cpseo' ), [ 'type' => 'error' ] );
			return false;
		}
	...
	private function set_options( $data ) {
		$set  = false;
		$hash = [
			'modules' => 'cpseo_modules',
			'general' => 'cpseo-options-general',
			'titles'  => 'cpseo-options-titles',
			'sitemap' => 'cpseo-options-sitemap',
		];
	...
	private function get_export_data( array $panels = [] ) {
		...
		$settings = cpseo()->settings->all_raw();
		...
		$data['modules'] = get_option( 'cpseo_modules', [] );
