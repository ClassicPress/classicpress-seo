...
abstract class Plugin_Importer {
	...
	public function run_import( $perform ) {
		if ( ! method_exists( $this, $perform ) ) {
			throw new Exception( esc_html__( 'Unable to perform action this time.', 'cpseo' ) );
		}
	...
	protected function is_twitter_using_facebook( $object_type, $object_id ) {
		$keys = [
			'cpseo_twitter_title',
			'cpseo_twitter_description',
			'cpseo_twitter_image',
		];

		foreach ( $keys as $key ) {
			if ( ! empty( $this->get_meta( $object_type, $object_id, $key, true ) ) ) {
				$this->update_meta( $object_type, $object_id, 'cpseo_twitter_use_facebook', 'off' );
				break;
			}
		}
	}
