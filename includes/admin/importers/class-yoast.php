...
class Yoast extends Plugin_Importer {
	...
	/**
	 * Convert Yoast SEO variables if needed.
	 *
	...
	protected function settings() {
		...
		// Titles & Descriptions.
		$hash = [
			'title-home-wpseo'       => 'cpseo_homepage_title',
			'metadesc-home-wpseo'    => 'cpseo_homepage_description',
			'title-author-wpseo'     => 'cpseo_author_archive_title',
			'metadesc-author-wpseo'  => 'cpseo_author_archive_description',
			'title-archive-wpseo'    => 'cpseo_date_archive_title',
			'metadesc-archive-wpseo' => 'cpseo_date_archive_description',
			'title-search-wpseo'     => 'cpseo_search_title',
			'title-404-wpseo'        => 'cpseo_404_title',
		];
	...
	protected function postmeta() {
		...
		$hash = [
			'_yoast_wpseo_title'                 => 'cpseo_title',
			'_yoast_wpseo_metadesc'              => 'cpseo_description',
			'_yoast_wpseo_focuskw'               => 'cpseo_focus_keyword',
			'_yoast_wpseo_canonical'             => 'cpseo_canonical_url',
			'_yoast_wpseo_opengraph-title'       => 'cpseo_facebook_title',
			'_yoast_wpseo_opengraph-description' => 'cpseo_facebook_description',
			'_yoast_wpseo_twitter-title'         => 'cpseo_twitter_title',
			'_yoast_wpseo_twitter-description'   => 'cpseo_twitter_description',
			'_yoast_wpseo_bctitle'               => 'cpseo_breadcrumb_title',
		];

		foreach ( $post_ids as $post ) {
			$post_id = $post->ID;
			$this->replace_meta( $hash, null, $post_id, 'post', 'convert_variables' );
			delete_post_meta( $post_id, 'cpseo_permalink' );
	...
	private function set_post_robots( $post_id ) {
		...
		if ( empty( $robots_nofollow ) && empty( $robots_noindex ) && empty( $robots_advanced ) ) {
			update_post_meta( $post_id, 'cpseo_robots', [] );
			return;
		}
	...
	private function set_post_social_media( $post_id ) {
		...
		if ( ! empty( $twitter_thumb ) ) {
			$this->replace_image( $twitter_thumb, 'post', 'cpseo_twitter_image', 'cpseo_twitter_image_id', $post_id );
		}
	}
	...
	private function set_post_focus_keyword( $post_id ) {
		...
		$main_fk   = get_post_meta( $post_id, 'cpseo_focus_keyword', true );
		update_post_meta( $post_id, 'cpseo_focus_keyword', $main_fk . ', ' . $extra_fks );
	}
	...
	private function set_primary_term( $post_ids ) {
		...
		foreach ( $results as $result ) {
			$key = str_replace( '_yoast_wpseo', 'cpseo', $result->meta_key );
	...
	protected function termmeta() {
		...
		$hash = [
			'wpseo_title'                 => 'cpseo_title',
			'wpseo_desc'                  => 'cpseo_description',
			'wpseo_metadesc'              => 'cpseo_description',
			'wpseo_focuskw'               => 'cpseo_focus_keyword',
			'wpseo_canonical'             => 'cpseo_canonical_url',
			'wpseo_opengraph-title'       => 'cpseo_facebook_title',
			'wpseo_opengraph-description' => 'cpseo_facebook_description',
			'wpseo_twitter-title'         => 'cpseo_twitter_title',
			'wpseo_twitter-description'   => 'cpseo_twitter_description',
			'wpseo_bctitle'               => 'cpseo_breadcrumb_title',
		];
		foreach ( $taxonomy_meta as $terms ) {
			foreach ( $terms as $term_id => $data ) {
				$count++;
				delete_term_meta( $term_id, 'cpseo_permalink' );
	...
	private function set_term_robots( $term_id, $data ) {
		...
		if ( ! empty( $data['wpseo_noindex'] ) && 'default' !== $data['wpseo_noindex'] ) {
			...
			update_term_meta( $term_id, 'cpseo_robots', [ $robots ] );
		}
	}
	...
	private function set_term_social_media( $term_id, $data ) {
		if ( ! empty( $data['wpseo_opengraph-image'] ) ) {
			$this->replace_image( $data['wpseo_opengraph-image'], 'term', 'cpseo_facebook_image', 'cpseo_facebook_image_id', $term_id );
		}

		if ( ! empty( $data['wpseo_twitter-image'] ) ) {
			$this->replace_image( $data['wpseo_twitter-image'], 'term', 'cpseo_twitter_image', 'cpseo_twitter_image_id', $term_id );
		}
	}
	...
	protected function usermeta() {
		...
		foreach ( $user_ids as $user ) {
			...
			// Early bail if robots data is set in Classic SEO plugin.
			if ( empty( $this->get_meta( 'user', $userid, 'cpseo_robots' ) ) && get_user_meta( $userid, 'wpseo_noindex_author', true ) ) {
				update_user_meta( $userid, 'cpseo_robots', [ 'noindex' ] );
			}
		}
