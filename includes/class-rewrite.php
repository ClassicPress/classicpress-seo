...
class Rewrite {
	...
	public function __construct() {
		...
		if ( Helper::get_settings( 'general.cpseo_strip_category_base' ) ) {
	...
	public function author_link( $link, $author_id, $author_nicename ) {
		$custom_url = get_user_meta( $author_id, 'cpseo_permalink', true );
		if ( $custom_url ) {
	...
	public function author_request( $query_vars ) {
		...
		$author_id = $wpdb->get_var( $wpdb->prepare( "SELECT user_id FROM {$wpdb->usermeta} WHERE meta_key='cpseo_permalink' AND meta_value = %s", $query_vars['author_name'] ) );
		if ( $author_id ) {
	...
	public static function change_author_base() {
		 ...
		 */
		$base = apply_filters( 'cpseo/author_base', sanitize_title_with_dashes( Helper::get_settings( 'titles.cpseo_url_author_base' ), '', 'save' ) );
		if ( empty( $base ) ) {
			return;
		}
	...
	public function query_vars( $query_vars ) {
		$query_vars[] = 'cpseo_category_redirect';
	...
	public function request( $query_vars ) {
		if ( isset( $query_vars['cpseo_category_redirect'] ) ) {
			$catlink = trailingslashit( get_option( 'home' ) ) . user_trailingslashit( $query_vars['cpseo_category_redirect'], 'category' );
	...
	public function category_rewrite_rules() {
		...
		$category_rewrite[ $old_base . '$' ] = 'index.php?cpseo_category_redirect=$matches[1]';
