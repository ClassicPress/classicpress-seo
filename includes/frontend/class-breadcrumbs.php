...
class Breadcrumbs {
	...
	public static function get() {
		...
		$instance = false;
		if ( Helper::get_settings( 'general.cpseo_breadcrumbs' ) && false === $instance ) {
	...
	public function __construct() {
		$this->settings = wp_parse_args(
			...
			[
				'home'            => Helper::get_settings( 'general.cpseo_breadcrumbs_home' ),
				'separator'       => Helper::get_settings( 'general.cpseo_breadcrumbs_separator' ),
		...
		$this->strings = wp_parse_args(
			...
			[
				'prefix'         => Helper::get_settings( 'general.cpseo_breadcrumbs_prefix' ),
				'home'           => Helper::get_settings( 'general.cpseo_breadcrumbs_home_label' ),
				'home_link'      => Helper::get_settings( 'general.cpseo_breadcrumbs_home_link', home_url() ),
				'error404'       => Helper::get_settings( 'general.cpseo_breadcrumbs_404_label' ),
				/* translators: search query */
				'search_format'  => Helper::get_settings( 'general.cpseo_breadcrumbs_search_format' ),
				/* translators: archive title */
				'archive_format' => Helper::get_settings( 'general.cpseo_breadcrumbs_archive_format' ),
			]
		);
	}
	...
	public function get_breadcrumb( $args = [] ) {
		$args = $this->do_filter(
			...
			wp_parse_args(
				...
				[
					'delimiter'   => '&nbsp;&#47;&nbsp;',
					'wrap_before' => '<nav class="cpseo-breadcrumb"><p>',
					'wrap_after'  => '</p></nav>',
					'before'      => '',
					'after'       => '',
				]
	...
	private function add_crumbs_product() {
		...
		$main_tax = Helper::get_settings( 'titles.cpseo_pt_product_primary_taxonomy' );
		if ( $main_tax ) {
	...
	private function add_crumbs_product_tag() {
		...
		$this->add_crumb( sprintf( __( 'Products tagged &ldquo;%s&rdquo;', 'cpseo' ), $this->get_breadcrumb_title( 'term', $term, $term->name ) ) );
	}
	...
	private function maybe_add_page_crumb() {
		...
		$this->add_crumb( sprintf( esc_html__( 'Page %s', 'cpseo' ), $current_page ), '', true );
	}
