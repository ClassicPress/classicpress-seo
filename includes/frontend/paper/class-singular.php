...
class Singular implements IPaper {
	...
	protected function get_post_title( $object = null ) {
		...
		return Paper::get_from_options( "cpseo_pt_{$post_type}_title", $object, '%title% %sep% %sitename%' );
	}
	...
	protected function get_post_description( $object = null ) {
		...
		return wp_html_excerpt( Paper::get_from_options( "cpseo_pt_{$post_type}_description", $object ), 160 );
	}
	...
	protected function get_post_robots( $object = null ) {
		...
		if ( empty( $robots ) && Helper::get_settings( "titles.cpseo_pt_{$post_type}_custom_robots" ) ) {
			$robots = Paper::robots_combine( Helper::get_settings( "titles.cpseo_pt_{$post_type}_robots" ), true );
		}

		// Noindex these conditions.
		$noindex_private            = 'private' === $object->post_status;
		$no_index_subpages          = is_paged() && Helper::get_settings( 'titles.cpseo_noindex_paginated_pages' );
		$noindex_password_protected = ! empty( $object->post_password ) && Helper::get_settings( 'titles.cpseo_noindex_password_protected' );
