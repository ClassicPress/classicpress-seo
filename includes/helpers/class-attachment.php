...
trait Attachment {
	...
	public static function get_relative_path( $img ) {
		...
		// If it's a relative URL, it's relative to the domain, not necessarily to the ClassicPress install, we
	...
	private static function url_to_postid( $url ) {
		$cache_key = sprintf( 'cpseo_attachment_url_post_id_%s', md5( $url ) );
		...
		// phpcs:ignore ClassicPress.VIP.RestrictedFunctions -- We use the WP COM version if we can, see above.
