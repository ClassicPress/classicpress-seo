...
trait Taxonomy {
	...
	public static function is_taxonomy_indexable( $taxonomy ) {
		if ( Helper::get_settings( 'titles.cpseo_tax_' . $taxonomy . '_custom_robots' ) ) {
			if ( in_array( 'noindex', (array) Helper::get_settings( 'titles.cpseo_tax_' . $taxonomy . '_robots' ), true ) ) {
				return false;
			}
		}

		return Helper::get_settings( 'sitemap.cpseo_tax_' . $taxonomy . '_sitemap' );
	}
	...
	public static function get_allowed_taxonomies() {
		static $cpseo_allowed_taxonomies;

		if ( isset( $cpseo_allowed_taxonomies ) ) {
			return $cpseo_allowed_taxonomies;
		}

		$cpseo_allowed_taxonomies = [];
		foreach ( self::get_accessible_taxonomies() as $taxonomy => $object ) {
			if ( false === Helper::get_settings( 'titles.cpseo_tax_' . $taxonomy . '_add_meta_box' ) ) {
				continue;
			}

			$cpseo_allowed_taxonomies[] = $taxonomy;
		}
	...
	public static function get_object_taxonomies( $post_type, $output = 'choices', $filter = true ) {
		...
		return empty( $taxonomies ) ? false : [ 'off' => esc_html__( 'None', 'cpseo' ) ] + wp_list_pluck( $taxonomies, 'label', 'name' );
	}

	/**
	 * Filter taxonomies using
	 *        `is_taxonomy_viewable` function
	 *        'cpseo_excluded_taxonomies' filter
	 *
	 * @codeCoverageIgnore
	 *
	...
	public static function filter_exclude_taxonomies( $taxonomies, $filter = true ) {
		...
		/**
		 * Filter: 'cpseo_excluded_taxonomies' - Allow changing the accessible taxonomies.
		 *
		 * @api array $taxonomies The public taxonomies.
		 */
		$taxonomies = apply_filters( 'cpseo/excluded_taxonomies', $taxonomies );
