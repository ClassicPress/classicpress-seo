...
class KML_File {
	...
	public function __construct() {
		...
		$this->filter( 'cpseo/sitemap/index', 'add_local_sitemap' );
		$this->filter( 'cpseo/sitemap/local/content', 'local_sitemap_content' );
		$this->filter( 'cpseo/sitemap/locations/content', 'kml_file_content' );
		$this->action( 'cmb2_save_options-page_fields_cpseo-options-titles_options', 'update_sitemap', 25, 2 );
	}
	...
	public function update_sitemap( $object_id, $updated ) {
		$local_seo_fields = [
			'cpseo_knowledgegraph_name',
			'cpseo_url',
			'cpseo_email',
			'cpseo_local_address',
			'cpseo_local_business_type',
			'cpseo_opening_hours',
			'cpseo_phone_numbers',
			'cpseo_price_range',
			'cpseo_geo',
	...
	private function get_local_seo_data() {
		...
		$locations = [
			'business_name'        => Helper::get_settings( 'titles.cpseo_knowledgegraph_name' ),
			'business_description' => get_option( 'blogname' ) . ' - ' . get_option( 'blogdescription' ),
			'business_email'       => Helper::get_settings( 'titles.cpseo_email' ),
			'business_phone'       => $number,
			'business_url'         => Helper::get_settings( 'titles.cpseo_url' ),
	...
	private function get_modified_date() {
		if ( ! $date = get_option( 'cpseo_local_seo_update' ) ) { // phpcs:ignore
			$date = date_i18n( 'c' );
		}
