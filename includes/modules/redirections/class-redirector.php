...
class Redirector {
	...
	private function fallback() {
		...
		$behavior = Helper::get_settings( 'general.cpseo_redirections_fallback' );
		if ( 'default' === $behavior ) {
			return;
		}
		...
		$custom_url = Helper::get_settings( 'general.cpseo_redirections_custom_url' );
		if ( ! empty( $custom_url ) ) {
			$this->matched     = [];
	...
	private function get_header_code() {
		$header_code = isset( $this->matched['header_code'] ) ? $this->matched['header_code'] : Helper::get_settings( 'general.cpseo_redirections_header_code' );
