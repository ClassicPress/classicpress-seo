...
class Cache_Watcher {
	...
	public function __construct() {
		...
		self::register_clear_on_option_update( 'cpseo_modules' );
		self::register_clear_on_option_update( 'cpseo-options-titles' );
		self::register_clear_on_option_update( 'cpseo-options-general' );
		self::register_clear_on_option_update( 'cpseo-options-sitemap' );
	}
	...
	public function status_transition( $new_status, $old_status, $post ) {
		...
		if ( WP_CACHE ) {
			wp_schedule_single_event( ( time() + 300 ), 'cpseo/sitemap/hit_index' );
		}
	...
	public function status_transition_bulk_finished() {
		...
		if ( WP_CACHE ) {
			do_action( 'cpseo/sitemap/hit_index' );
		}
	...
	public function invalidate_term( $unused, $taxonomy ) {
		if ( false !== Helper::get_settings( 'sitemap.cpseo_tax_' . $taxonomy . '_sitemap' ) ) {
