...
class Generator extends XML {
	...
	public function build_sitemap( $type, $page ) {
		$this->max_entries = absint( Helper::get_settings( 'sitemap.cpseo_items_per_page', 100 ) );
	...
	public function get_sitemap( $links, $type, $current_page ) {
		...
		foreach ( $links as $url ) {
			...
			$xml   .= has_filter( "cpseo/sitemap/{$method}" ) ? $this->do_filter( "sitemap/{$method}", $url, $this ) : $this->sitemap_url( $url );
		}
